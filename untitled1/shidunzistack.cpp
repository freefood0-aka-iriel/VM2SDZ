...
void ShidunziStack::read_as_VM(const std::string& precontent, std::vector<ExStone>& exs, std::vector<ExTouch>& ext, SongInfomation& SongInfo, int d)
{
    ...
    std::string content;
    size_t diff_start = precontent.find(("\"difficulty\":"+std::to_string(d)));

    if (diff_start != std::string::npos)
        content = precontent.substr(diff_start, std::string::npos);
    else
        content = precontent;
    ...
    size_t bpm_start = content.find("\"keyframes\":");    //寻找bpm列表
    if (bpm_start != std::string::npos)
    {
        bpm_start += 10;
        size_t bpm_end = content.find(']', bpm_start);
        std::string bpm_str = content.substr(bpm_start, bpm_end - bpm_start + 2);
